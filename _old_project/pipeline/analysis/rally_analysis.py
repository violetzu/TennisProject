    ...
    for i, contact in enumerate(contacts):
        if current_rally_contacts:
            ...
            if frame_gap > gap_frames:
                ...
                #查看是在哪裡換回合的
                # ==========================================================
                actual_gap_sec = frame_gap / fps
                print(f"[Debug] ⚠️ 觸發換回合 (Rally {rally_id} -> {rally_id + 1})")
                print(f"   - 上一拍結束時間 : {last_contact.time:.2f} 秒 (Frame {last_contact.frame_index})")
                print(f"   - 新一拍開始時間 : {contact.time:.2f} 秒 (Frame {contact.frame_index})")
                print(f"   - 兩者時間間隔   : {actual_gap_sec:.2f} 秒 (大於設定的門檻 {gap_threshold_sec} 秒)")
                print(f"   - 上一拍 Y 座標  : {last_contact.y:.2f}m, 新一拍 Y 座標: {contact.y:.2f}m")
                print("-" * 50)
                # ==========================================================
...
def main():
    ...
    parser.add_argument("--gap", type=float, default=3.5, help="Gap threshold in seconds")
