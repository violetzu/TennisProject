...
class AlternatingDetectorConfig:
    ...
    min_velocity_change: float = 6.5  # m/s - any significant change (raised from 3.0)
    min_velocity_magnitude: float = 4.0  # m/s - ball must be moving (raised from 3.0)
    
    # Timing
    cooldown_frames: int = 15  # Increased from 8 to prevent consecutive same-type events
    
    # Player proximity for contact classification
    player_proximity_threshold: float = 5.0  # meters (reduced from 5.0 for stricter contact)
