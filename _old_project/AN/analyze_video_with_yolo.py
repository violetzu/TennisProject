...
def analyze_video_with_yolo(
    ...
    # 若要把 AN 當主 pipeline，後續要把球軌跡餵給另一段 pipeline（world/speed/event），
    # 就需要把每一幀的 chosen_box/mask 收集起來。
    all_ball_boxes: list = []
    all_ball_mask: list = []
    ...
    try:
        while True:
            ...
            # 收集每幀球追蹤輸出（給後續 pipeline 使用）
            all_ball_boxes.append(chosen_box)
            all_ball_mask.append(chosen_box is not None)
    ...
    if return_tracks:
        return {
            "overlay_video": str(out_path),
            "ball_boxes": all_ball_boxes,
            "detection_mask": all_ball_mask,
            "fps": fps,
            "width": width,
            "height": height,
        }
