...
from config import BALL_MODEL_PATH, POSE_MODEL_PATH, COURT_MODEL_PATH

def get_yolo_models(device: str | None = None) -> tuple[YOLO, YOLO, YOLO]:
    for p, name in [(BALL_MODEL_PATH, "球偵測"), (POSE_MODEL_PATH, "姿態"),
                    (COURT_MODEL_PATH, "場地")]:
        if not p.exists():
            raise FileNotFoundError(f"找不到{name}模型檔案：{p}")
    ...
    court_model = YOLO(str(COURT_MODEL_PATH)).to(device)

    return ball_model, pose_model, court_model
