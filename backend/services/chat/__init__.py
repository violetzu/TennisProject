...
class _ToolCallAccum:
    ...
    id:    str  = ""
    name:  str  = ""
    args:  str  = ""   # JSON 字串，累積後 json.loads
...
class ChatService:
    ...
    def _stream_analyzed(
        ...
        # 解析 SSE 串流，先緩衝 content；若最後沒有 tool call 才整段輸出。
        ...
        # 若沒有 tool call → 狀態 B 完成
        if not tool_accums:
            for chunk in content_chunks:
                yield self._message_event(chunk)
            return

        # 狀態 C：有 tool call，執行工具後再呼叫 LLM
        ...
        for tc in sorted(tool_accums.values(), key=lambda x: x.index):
            ...
            # 目前只支援一個 tool call（多個時只用第一個）
            break

    # ── 訊息建構（text-only，無 video_url）────────────────────────────────────
